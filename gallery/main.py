...
def mainGame():
    ...
    basex = 0
    # creating 2 pipes at random location
    ...
    playerFlapAcc = -8  # velocity whille flapping
    playerFlapped = False  # true when bird is flappng
...
def getRandomPipe():
    # generate pipes funcn
    pipeHeight = GAME_SPRITES['pipe'][0].get_height()
    offset = SCREENHEIGHT / 3
    y2 = offset + random.randrange(0,
                                   int(SCREENHEIGHT - GAME_SPRITES['base'].get_height() - 1.2 * offset))  # lower pipe
    pipeX = SCREENWIDTH + 10
    y1 = pipeHeight - y2 + offset  # upper pipe
    pipe = [
        {'x': pipeX, 'y': -y1},
        {'x': pipeX, 'y': y2}
    ]
